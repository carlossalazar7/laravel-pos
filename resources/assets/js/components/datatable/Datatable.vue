<template>
    <div>
        ...
        <export-data
            :export-excel-show="exportExcelShow"
            :export-excel-data="exportExcelData"
            :column-header="options.columns"
            :excel-file-name="exportFileName"
            @resetExport="resetExcel"
        ></export-data>
                        ...
                        if (instance.paginationLimitIncreased) {
                            ...
                            if (instance.options.summary) {
                                instance.datasets = instance.datasets.filter(function (element) {
                                    return element[instance.options.summationKey] !== instance.trans('lang.total') && element[instance.options.summationKey] !== instance.trans('lang.grand_total');
                                });
                                instance.options.summation.forEach(function (name) {
                                    let match = _.find(newData, function (item) {
                                        return item[instance.options.summationKey] === instance.trans('lang.total')
                                    });
                                    if (match) {
                            ...
                            if (instance.options.summary) {
                                instance.totalDataRow = _.clone(response.data.datarows.find(key => key[instance.options.summationKey] === instance.trans('lang.total')));
                            }
