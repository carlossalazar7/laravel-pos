<template>
  <div class="side-bar custom-scrollbar">
    <ul class="m-0">
      ...
      <li
          v-if="isGuide"
          :class="{'active-side-bar': currentUrl == publicPath+'/guide', 'active-hover': isConnected || offline == 0}"
      >
        <a :href="publicPath+'/guide'" :class="{disabled:!isConnected && offline == 1}">
          <i class="la la-clipboard-list"></i>
          <div>{{ trans('lang.guides') }}</div>
        </a>
      </li>
      <li
