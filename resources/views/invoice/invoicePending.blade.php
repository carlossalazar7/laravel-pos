    ...
    <div id="cart-print">
            ...
            <table style="border-collapse: collapse !important; width: 65% !important; line-height: .5em; margin-bottom: -1em;">
                <tr>
                    <td>
                        ...
                        <p>
                            <span class="font-weight-bold mb-2">@lang('custom.shipping_address'):</span>
                            @foreach($order->shippingInfo as $item)
                        <p>{{$item->departamento}}, {{$item->municipio}}</p>
                        <p>{{$item->shipping_address}}</p>
                        @endforeach
                        </p>
                        <p>
                            <span class="font-weight-bold mb-2">@lang('custom.delivery_note'):</span>
                            @foreach($order->shippingInfo as $item)
                            @if ($item->delivery_note)
                        <p>{{$item->delivery_note}}</p>
                        @else
                        <p>@lang('custom.delivery_note_not_found')</p>
                        @endif
                        @endforeach
                        </p>
                    </td>
        ...
        <div class="invoice-table" style="line-height: 1em;">
            <table class="table product-card-font">
                ...
                <tfoot>
                <tr>
                    <td class="font-weight-bold" colspan="4">@lang('default.sub_total')</td>
                    <td class=" text-right">{{$order->sub_total}}</td>
                </tr>
                <tr>
                    <td class="font-weight-bold" colspan="4">@lang('default.total')</td>
                    <td class="text-right">{{$order->total}}</td>
                </tr>
                <tr>
                    <td class="font-weight-bold" colspan="4">@lang('default.method')</td>
                    <td class="text-right">{{$order->total}}</td>
                </tr>
                <tr>
                    <td class="font-weight-bold" colspan="4">@lang('default.due')</td>
